...
def get_records_from_db(host, port, database, user, password, product, date, limit):
    ...
    if limit is None:
        query = f"SELECT product_name,product_id,creation_date FROM ingested_product where status='INGESTED' and product_name like '{product}' and creation_date > '{date}T00:00:00' and creation_date < '{date}T23:59:59' order by creation_date ASC;"
    
    else:
        query = f"SELECT product_name,product_id,creation_date FROM ingested_product where status='INGESTED' and product_name like '{product}' and creation_date > '{date}T00:00:00' and creation_date < '{date}T23:59:59' order by creation_date ASC limit {limit};"
...
@click.option('-l', '--limit', 'limit', type=str, help='Limit number of records i.e. 10')
def main(catalogue_config, download_dir, rsync_user, rsync_host, product_string, product_date, output_file, limit=None):
    ...
    with open(output_file, 'w') as fp:
        for file_to_rsync in records:
            ...
            #construct rsync commands
            fp.writelines(f"rsync -ssh {rsync_user}@{rsync_host}:{data_file} {download_dir}\n")
            fp.writelines(f"rsync -ssh {rsync_user}@{rsync_host}:{checksum_file} {download_dir}\n")
