...
class Sentinel_Product(object):
    ...
    SENSING_DATE_PARAMS = {'S1A':'17:32', 'S1B':'17:32', 'S2A':{'S2A_OPER_PRD_MSIL1C':'25:40', 'S2A_MSIL1C':'11:26','S2A_MSIL2A':'11:26'},\
                           'S2B':{'S2B_OPER_PRD_MSIL1C':'25:40', 'S2B_MSIL1C':'11:26','S2A_MSIL2A':'11:26'}, 'S3A':'16:30', 'S3B':'16:30'}
    ...
    PRODUCT_NAME_PARAMS = {'S1A':'0:11', 'S1B':'0:11', 'S2A':{'S2A_OPER_PRD_MSIL1C':'0:19', 'S2A_MSIL1C':'0:10', 'S2A_MSIL2A':'0:10'}, \
                           'S2B':{'S2B_OPER_PRD_MSIL1C':'0:19', 'S2B_MSIL1C':'0:10', 'S2A_MSIL2A':'0:10'}, 'S3A':'0:11', 'S3B':'0:11'}
    ...
    def get_product_date(self):
        ...
        if type(self.SENSING_DATE_PARAMS[self.mission]) is dict:
            for sub_type in list(self.PRODUCT_NAME_PARAMS[self.mission].keys()):
...
def extract_synchronizer_details(line, start_date= None, end_date=None ):
    ...
    if SYNC_SUCCESS_TEXT_SNIPPET in line:
        ...
        '''
    #else:
        #Another synchroniser line ie Synchroniser "done message".  Can ignore these for this management_scripts purposes
        #entry = None
...
def download_report(logfiles):
    ...
    for user in list(overview_by_user.keys()):
    ...
    for user in list(download_volume.keys()):
    ...
    for user in list(download_products_volume.keys()):

        #now loop over on a per product basis
        for product in list(download_products_volume[user].keys()): #note keys are the same for both these dicts
    ...
    report += "\nTotal number of users active: %s" %len(list(download_volume.keys()))
...
def summarise_downloads_per_product_per_sync(per_sync,per_sync_fail):
    ...
    for sync in sorted(per_sync.keys()):
        ...
        for product in list(per_sync[sync].keys()):
...
def synchronizer_report(logfiles, write_products = None):
    ...
    for sync in list(per_sync.keys()):
        sync_total = 0.
        sync_total_prods = 0.
...
def eviction_report(logfiles, write_products = None):
    ...
    evicted_total = 0.
    for product in list(eviction_overview.keys()):
    ...
    if len(list(eviction_overview.keys())) != 0:
